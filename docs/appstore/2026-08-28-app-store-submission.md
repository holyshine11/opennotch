...
- **Accessibility (optional):** the setup window offers “Open the menu for me”. If the user grants Accessibility, the app uses the public Accessibility API only to open the browser's View › Developer menu (and to tick Safari's ‘Allow JavaScript from Apple Events’ checkbox in Safari Settings). Nothing on screen is read or recorded; the feature is fully optional and the manual path is always shown. macOS does not show the Accessibility prompt for sandboxed apps, so the window guides the user to add OpenNotch to the list manually (System Settings › Privacy & Security › Accessibility › +).
